...
export default function SolarSystemHero() {
  return (
    <section className="relative min-h-[calc(100vh-6rem)] overflow-hidden flex items-center bg-brand-dark pt-8 pb-16">
      {/* Background Elements */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 w-full relative z-10 flex flex-col lg:flex-row items-center">
        ...
        {/* Left Content */}
        <div className="lg:w-1/2 space-y-8 z-20 text-center lg:text-left mt-10 lg:mt-0">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8 }}
          >
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/5 border border-white/10 text-xs font-bold text-brand-cyan uppercase tracking-widest shadow-glass mb-6">
              DIGI8 SOLUTIONS INDIA PRIVATE LIMITED
            </div>
            
            <h1 className="font-outfit font-black text-5xl sm:text-6xl lg:text-7xl text-white leading-[1.1] tracking-tight">
              Transform Your Business Into A <span className="text-gradient">Digital Powerhouse</span>
            </h1>
            
            <p className="text-slate-300 text-lg sm:text-xl max-w-xl mx-auto lg:mx-0 mt-6 font-inter leading-relaxed">
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-8 justify-center lg:justify-start">
              <Link to="#services" className="btn-glow py-4 px-8 text-sm font-bold">
                Explore Services
              </Link>
              <Link to="/contact" className="btn-outline-glass py-4 px-8 text-sm font-bold">
