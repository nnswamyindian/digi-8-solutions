...
export default function Careers() {
  ...
  const perks = [
    { title: 'Remote-Friendly', desc: 'Work from anywhere in India with flexible hours.', icon: '🌍' },
    { title: 'Learning Budget', desc: 'Annual budget for courses, books and conferences.', icon: '📚' },
    { title: 'Health Insurance', desc: 'Comprehensive medical cover for you and family.', icon: '❤️' },
    { title: 'Equity Options', desc: 'ESOPs for senior roles and long-term contributors.', icon: '📈' },
    { title: 'Annual Retreat', desc: 'Company-wide retreat to celebrate wins together.', icon: '🏝️' },
    { title: 'MacBook Pro', desc: 'Latest Apple hardware from day one for all engineers.', icon: '💻' },
  ];
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {perks.map((perk, i) => (
              <div key={perk.title} className="glass-card-premium p-8 group hover:-translate-y-2 transition-transform duration-300">
                <div className="text-4xl mb-6 opacity-80 group-hover:opacity-100 group-hover:scale-110 transition-all">{perk.icon}</div>
